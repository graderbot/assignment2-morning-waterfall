...
// echoPathCaptures writes the raw url query of the reqeust to the response body, but
// otherwise ignores all other request fields
func echoPathCaptures(response http.ResponseWriter, request *http.Request) {
	captureBytes := []byte(request.URL.RawQuery)
	response.Write(captureBytes)
}
